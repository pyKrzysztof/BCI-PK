...
# Setup global socket_thread, fft and working parameters
socket_thread = None
fft_freq_axis = None
fft_data_size = None
live_plot = True
live_predict = False
store_connection_data = True
...
def process_data(data_queue: queue.Queue, curves, fft_curves, time_data, channel_data, samples_to_display, fft_data_size, fft_freq_axis, csv_writer):
    try:
        ...
        # TODO Extend the functionality to also save gyroscope data (for movement artifact/data rejection)
        for samples in zip(*data[:ELECTRODES], marker_data): # gyro_data):
            row = [samples[-1],]
