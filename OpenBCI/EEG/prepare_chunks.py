...
# Load the CSV file
file_path = 'data/raw_session_data/2_processed.csv'
out_path = 'data/chunks_processed/2/'
...
for index, row in df.iterrows():
    ...
    if last_col_value == 1:
        ...
        chunk_df.to_csv(os.path.join(out_path, f'L{L_counter}.csv'), header=False, index=False)
        ...
        # Save the chunk to a file
        chunk_df = pd.DataFrame(current_chunk)
        chunk_df.to_csv(os.path.join(out_path, f'R{R_counter}.csv'), header=False, index=False)
