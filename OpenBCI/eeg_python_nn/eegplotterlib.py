...
class AppBase:
    ...
    def __init__(self, board_shim: BoardShim=None, from_file="", update_speed_ms=50, points_to_analyze=32, display_s=4):
        ...
        if from_file != "":
            ...
            timer.timeout.connect(self.update_file)
        else:
...
class AppAnalyzer(AppBase):
    ...
    def __init__(self, board_shim: BoardShim=None, from_file="", update_speed_ms=50, points_to_analyze=32, display_s=4, processes=[], fft_size=256, y_limit=0):
    ...
    def init_timeseries(self):
        ...
        for i in range(len(self.eeg_channels)):
            p = self.win.addPlot(row = i if self.source_file == "" else i + 1, col=0)
            p.showAxis('left', True)
            p.setMenuEnabled('left', False)
            p.showAxis('bottom', False)
            p.setMenuEnabled('bottom', False)
            if self.y_limit != 0:
                p.setYRange(-self.y_limit, self.y_limit, padding=5)
                p.vb.setLimits(yMin=-self.y_limit, yMax=self.y_limit)
            self.plots.append(p)
    ...
    def init_fft(self):
        ...
        for i in range(len(self.processes)):
            j = i if self.source_file == "" else i+1
            plot = self.win.addPlot(row=j if i == 0 else j*increment, col=1, rowspan=increment)
    ...
    def update(self):
        ...
        # plot timeseries
        for curves_ts, display_data in zip(self.curves, self.full_data):
            for count, channel in enumerate(self.eeg_channels):
    ...
    def update_file(self):
        ...
        for i in range(len(self.processes)):
            for count, channel in enumerate(self.eeg_channels):
                self.curves[i][count].setData(self.full_data[i][channel][idx_plot_start:idx_plot_end])
