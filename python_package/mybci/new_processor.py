...
class SystemConfig:
    ...
    def create_config_file(self):
        data = f"""from mybci.new_processor import SystemConfig
from mybci.feature_extraction import Filters, Features
...
config.board_type = {self.board_type}
...
class DataReader:
    ...
    def __init__(self, data_source: BoardShim | str, packet_size: int, buffer_size: int, return_overlapping: bool, sep='\t'):
...
class System:
    ...
    def create_system(cls):
        ...
        with open("run.sh", 'w') as f:
            f.writelines("python -i -c \"from mybci.new_processor import System;sys = System.load_system();print('System loaded into variable <sys>.')\"")
        st = os.stat("run.sh")
        os.chmod("run.sh", st.st_mode | stat.S_IXUSR | stat.S_IXGRP | stat.S_IXOTH)
        
    @classmethod
    def load_system(cls, path:str = None):
        """Loads saved system."""
        if not path:
            path = os.getcwd() + "/"
        system = System()
        ...
        with open(path + "system.bin", 'rb') as f:
            system.stats, system.files = pickle.load(f)

        system.create_filesystem() # in case config changed
        return system
    ...
    def create_filesystem(self):
        ...
        for action in self.config.actions:
            self.files.action_data[action] = os.path.join(self.files.data_path, action + "/")
            os.makedirs(os.path.join(self.files.action_data[action], "raw/"), exist_ok=True)
            os.makedirs(os.path.join(self.files.action_data[action], "processed/"), exist_ok=True)
